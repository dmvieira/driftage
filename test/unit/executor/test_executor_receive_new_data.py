...
class TestReceiveNewData(TestCase):
    ...
    @patch("driftage.executor.behaviour.receive_new_data.SendNewData")
    @patch("driftage.executor.behaviour.receive_new_data.Template")
    async def test_should_send_data_on_message(self, template, callback):
        message = Mock(1)
